...
% New interpretation for the power drop. 
%% ============================================================
    ...
    for v = 1:length(velocidades)
        ...
        for e = 1:length(estrategias)
            ...
            for sd = 1:length(seeds)
                ...
                M = compute_metrics(Time, P_pu, P_pre, t_event, estrategia, IS_status);
                if estrategia == "Tarnowski" && M.is_status_3 == 2.5
                    disp([Vstr,sd_str])
                end
                RawMetrics.(estrategia).(vel_field).(sd_str) = M;
            end
...
for e = 1:length(estrategias)
    ...
    for iv = 1:numel(vel_fields)
        ...
        GridCodes.IESO.(vel).max_post_drop_pct = FinalMetrics.(est).(vel).P_pre*0.1;
...
function M = compute_metrics(Time,P_pu,P_pre,t_event,estrategia,IS_status)
    ...
    mask_rec = IS_status==2 | IS_status==3 | IS_status==1.5 | IS_status==2.5;  %tanto status 2 como 3 son recuperaciones. En Wang (status 2 o 3) y en Tarnowski! Agrego 1.5 para casos en los que pasa directamente de 1.5 a 0. 
    
    test = find(IS_status==3, 1); a=0;
    if estrategia == "Tarnowski" && ~isempty(test)   %encontrar casos con recuperación forzada por aceleración en Tarnowski.
        disp('Found IS_status = 3');
        a=1;
    end
    test2 = find(IS_status==2.5, 1); %a=0;
    if estrategia == "Tarnowski" && ~isempty(test2)   %encontrar casos con recuperación forzada por aceleración en Tarnowski.
        disp('Found IS_status = 2.5');
        a=2.5;
    end
    ...
    if any(mask_rec)
        ...
        disp('no data in mask_rec')
        P_end = P_pu(end);
    end
    
    M.drop_pct_pre_dist = P_pre-P_end; %COrrige FG 18/03/2026 - P_end/P_pre;
    M.drop_pct_to_rtd   = P_end;
   ...
   % ylabel(metrics_labels{im},'Interpreter','latex');
    ...
    if im ==3
        ...
        b1 = bar(x, bar_data(:,1), 0.7, 'FaceColor', colores(1,:), ...
                 'EdgeColor','none','FaceAlpha',alpha_bar,'DisplayName','Stepwise');
             
        % ==== BARRA ESTRATEGIA 2 (ANGOSTA, ADELANTE) ====
        b2 = bar(x, bar_data(:,2), 0.4, 'FaceColor', colores(2,:), ...
                 'EdgeColor','none','FaceAlpha',alpha_bar,'DisplayName','GMFC');
        % ==== BARRA ESTRATEGIA 2 (ANGOSTA, ADELANTE) ====
        b3 = bar(x, bar_data(:,3), 0.2, 'FaceColor', colores(3,:), ...
                 'EdgeColor','none','FaceAlpha',alpha_bar,'DisplayName','Torque-limit');
