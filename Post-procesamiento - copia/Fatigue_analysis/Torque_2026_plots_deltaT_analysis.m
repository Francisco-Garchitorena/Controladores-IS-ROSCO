...
legend('5','10','GMFC','Location','northeast','FontSize',fs-4,'Interpreter','latex');
...
% estrategia_names = ["GMFC","SW"];
estrategias    = [ "Norm_op","GMFC_5","GMFC_10"];   % estrategias a comparar
estrategia_names = ["NO", "5", "10"];
...
for vvar = 1:nVar
    ...
    for e = 1:length(estrategias)
        ...
        for v = 1:length(velocidades)
            ...
            for sd = 1:length(seeds)
                sd_field = seeds(sd);
                if isfield(Maxs.(estrategia).(vel_field), sd_field)si
