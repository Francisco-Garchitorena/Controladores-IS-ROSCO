...
IF (((LocalVar%iStatus >= 0) .OR. (LocalVar%iStatus <= -8)) .AND. (ErrVar%aviFAIL >= 0))  THEN  ! Only compute control calculations if no error has occurred and we are not on the last time step
    ...
    CALL VariableSpeedControl(avrSWAP, CntrPar, LocalVar, DebugVar, objInst, ErrVar)
    CALL SyntheticInertia(avrSWAP, CntrPar, LocalVar, DebugVar, objInst, ErrVar, PerfData)
    !IF (LocalVar%n_DT < CntrPar%Itim_SI) THEN
    !    CALL VariableSpeedControl(avrSWAP, CntrPar, LocalVar, objInst, ErrVar)
    !ELSE
    !    CALL SyntheticInertia(avrSWAP, CntrPar, LocalVar, objInst, ErrVar)
    !END IF
