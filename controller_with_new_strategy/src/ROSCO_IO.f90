...
SUBROUTINE WriteRestartFile(LocalVar, CntrPar, ErrVar, objInst, RootName, size_avcOUTNAME)
    ...
    ELSE
        ...
        WRITE( Un, IOSTAT=ErrStat) LocalVar%IS_status        !FG
        WRITE( Un, IOSTAT=ErrStat) LocalVar%Pgen_ref         !FG
        WRITE( Un, IOSTAT=ErrStat) LocalVar%OmegaTi          !FG
        WRITE( Un, IOSTAT=ErrStat) LocalVar%IS_ControlMode2  !FG
        WRITE( Un, IOSTAT=ErrStat) LocalVar%Pmec_est         !FG: Estimated mechanical power 30/06/25
        WRITE( Un, IOSTAT=ErrStat) LocalVar%GenSpeedF_Last         !FG: Estimated mechanical power 30/06/25
        WRITE( Un, IOSTAT=ErrStat) LocalVar%GenSpeedF_dot_gmfc_strategy
        WRITE( Un, IOSTAT=ErrStat) LocalVar%GenTq_end !FG
        WRITE( Un, IOSTAT=ErrStat) LocalVar%T_end_rec_Tar         !FG 10/11
...
SUBROUTINE ReadRestartFile(avrSWAP, LocalVar, CntrPar, objInst, PerfData, RootName, size_avcOUTNAME, ErrVar)
    ...
    ELSE
        ...
        READ( Un, IOSTAT=ErrStat) LocalVar%IS_status   !FG
        READ( Un, IOSTAT=ErrStat) LocalVar%Pgen_ref    !FG
        READ( Un, IOSTAT=ErrStat) LocalVar%OmegaTi     !FG
        READ( Un, IOSTAT=ErrStat) LocalVar%IS_ControlMode2  !FG
        READ( Un, IOSTAT=ErrStat) LocalVar%GenSpeedF_dot_gmfc_strategy  !FG
        READ( Un, IOSTAT=ErrStat) LocalVar%GenTq_end  !FG
        READ( Un, IOSTAT=ErrStat) LocalVar%T_end_rec_Tar !FG 10/11
...
! Debug routine, defines what gets printed to DEBUG.dbg if LoggingLevel = 1
    ...
    nDebugOuts = 35 !14/11/25 agrego la VS_SpdErr ! 02/07/25: eran 26 agregue is status y Pmecest !!FG: /03/07/25: eran 28 agregue GenSpeedF_dot FG:25/08 agregue VS_GenTq !FG: /03/02/26: VS_state,komega2Tq
    ...
    DebugOutData(27) = DebugVar%IS_status
    DebugOutData(28) = DebugVar%Pmec_est
    DebugOutData(29) = DebugVar%GenSpeedF_dot
    DebugOutData(30) = DebugVar%Tau_r
    DebugOutData(31) = DebugVar%VS_GenTq
    DebugOutData(32) = DebugVar%VS_SpdErr
    DebugOutData(33) = DebugVar%VS_State
    DebugOutData(34) = DebugVar%komega2Tq
    DebugOutData(35) = DebugVar%GenTq
    DebugOutStrings = [CHARACTER(15) ::  'WE_Cp', 'WE_b', 'WE_w', 'WE_t', 'WE_Vm', & 
                                      ...
                                      'PC_RefSpd','IS_status', 'Pmec_est', 'GenSpeedF_dot', 'Tau_r','VS_GenTq','VS_SpdErr','VS_State','komega2Tq','GenTq']
    DebugOutUnits = [CHARACTER(15) ::  '[-]', '[-]', '[-]', '[-]', '[m/s]', & 
                                      '[m/s]', '[m/s]', '[rad]', '[rad]', '[rad/s]', & 
                                      '[rad/s]', '[rad/s]', '[m/s]', '[rad]', '[rad]', & 
                                      '[N/A]', '[N/A]', '[N/A]', '[N/A]', '[rad/s]', & 
                                      '[deg]', '[deg]', '[deg]', '[N/A]', '[rad/s]', & 
                                      '[rad/s]','[-]', '[W]', '[rad/s^s]', '[Nm]','[Nm]','[rad/s]', '[-]', '[Nm]', '[Nm]']
    nLocalVars = 133 ! cambio 30/06/25. Prev: 128--added pmec est and last_gen_speed !07/08 !08/08 !15/08
    ...
    LocalVarOutData(125) = LocalVar%IS_status
    LocalVarOutData(126) = LocalVar%Pgen_ref
    LocalVarOutData(127) = LocalVar%OmegaTi
    LocalVarOutData(128) = LocalVar%IS_ControlMode2
    LocalVarOutData(129) = LocalVar%Pmec_est 
    LocalVarOutData(130) = LocalVar%GenSpeedF_Last
    LocalVarOutData(130) = LocalVar%GenTq_Ti
    LocalVarOutData(131) = LocalVar%T_end_IS_Tar
    LocalVarOutData(132) = LocalVar%IS_LastGenTrq
    LocalVarOutData(132) = LocalVar%Last_omega
    LocalVarOutData(133) = LocalVar%T_end_rec_Tar       
    LocalVarOutStrings = [CHARACTER(15) ::  'iStatus', 'Time', 'DT', 'n_DT', 'Time_Last', & 
                                      ...
                                      'ZMQ_ID', 'ZMQ_YawOffset', 'ZMQ_TorqueOffset', 'ZMQ_PitOffset', 'IS_status', 'Pgen_ref', &
                                      'OmegaTi', 'IS_ControlMode2','Pmec_est','GenSpeedF_Last', 'GenTq_Ti', 'T_end_IS_Tar', 'IS_LastGenTrq', 'Last_omega', 'T_end_rec_Tar']
