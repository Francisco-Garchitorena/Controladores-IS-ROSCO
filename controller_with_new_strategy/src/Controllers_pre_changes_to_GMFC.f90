...
!-------------------------------------------------------------------------------------------------------------------------------
    ...
    !       VS_State = 6, Tip-Speed-Ratio tracking PI controller
        ...
        ELSEIF (CntrPar%VS_ControlMode == 1) THEN
            ...
            DebugVar%komega2Tq = CntrPar%VS_Rgn2K*LocalVar%GenSpeedF*LocalVar%GenSpeedF
            ! The action
            IF (LocalVar%VS_State == 1) THEN ! Region 1.5
                LocalVar%GenTq = LocalVar%GenBrTq
                DebugVar%VS_state = 1.5
    ...
    SUBROUTINE SI_Tarnowski_GMFC(CntrPar, LocalVar, DebugVar, PerfData, ErrVar)
       ...
       !         ELSEIF ((LocalVar%GenSpeedF > CntrPar%VS_RefSpd * CntrPar%F_omega_min) .AND. (LocalVar%IS_status == 0.5)) THEN
                ...
                ELSEIF ((LocalVar%GenSpeedF > CntrPar%VS_RefSpd * CntrPar%F_omega_min) .AND. (LocalVar%IS_status == 1)) THEN
                    ...
                    IF ((LocalVar%GenTq - LocalVar%IS_LastGenTrq) / LocalVar%DT > CntrPar%VS_MaxRat) THEN
                        PRINT *, ' Step 1: WARNING: Synthetic Inertia torque rate limit exceeded ', LocalVar%GenTq
                    ELSEIF (LocalVar%GenTq > CntrPar%VS_MaxTq) THEN
                      !  LocalVar%GenTq = CntrPar%VS_MaxTq
                        PRINT *, ' Step 1: WARNING: Synthetic Inertia torque limit exceeded ', LocalVar%GenTq
                    END IF

                    LocalVar%T_end_IS_Tar = LocalVar%n_DT * LocalVar%DT   ! Me va a guardar el paso en el que arranca la rampa de bajada
                    LocalVar%IS_status = 1
                    LocalVar%GenTq_ramp = LocalVar%GenTq

                    IF  ((LocalVar%n_DT >= CntrPar%Itim_SI + 1920) .AND. ((LocalVar%GenSpeedF - LocalVar%GenSpeedF_Last)>0)) THEN   ! Si pasaron X segundos y se está acelerando la turbina, salgo de la IS.
                        IF (LocalVar%IS_LastGenTrq >= LocalVar%VS_LastGenTrq) THEN
                            PRINT *, 'Acceleration deteceted: recovering with mechanical power estimator'
                            LocalVar%IS_status = 3
                        ELSE
                            PRINT *, 'Acceleration deteceted: recovering with VS Controller'    
                ...
                !Status 3: Turbina se empezó a acelerar porque Pgen menor que Pmec, pero velocidad de giro muy baja -- requiere recuperación hasta que me choque con el VS controller.
                ELSEIF ((LocalVar%IS_status == 3) .AND. (LocalVar%IS_LastGenTrq >= LocalVar%VS_LastGenTrq)) THEN    ! Si me estoy acelerando pero tengo velocidad de giro muy baja, recupero con Tarnowski para evitar valle de potencia.
                    
                    CntrPar%Frecovery = 0.08
                ...
                ELSEIF (LocalVar%IS_status == 1 .AND. &
                    ...
                    LocalVar%P_mec_init = (LocalVar%Pmec_est - (CntrPar%VS_RtPwr * CntrPar%Frecovery)) / LocalVar%GenSpeedF
                    LocalVar%VS_LastGenTrq_init_ramp = LocalVar%VS_LastGenTrq
                    PRINT *, ' Entre a la rampa de bajada', LocalVar%GenTq
                    IF (LocalVar%VS_LastGenTrq_init_ramp <= LocalVar%P_mec_init) THEN
                        LocalVar%GenTq = ( ( (( LocalVar%Pmec_est - (CntrPar%VS_RtPwr * CntrPar%Frecovery) )/ LocalVar%GenSpeedF) - &
                        ( LocalVar%GenTq_ramp ) ) / CntrPar%dt_rise * &
                        ( (LocalVar%n_DT * LocalVar%DT) - LocalVar%T_end_IS_Tar ) + &
                        (LocalVar%GenTq_ramp))
                        
                        LocalVar%IS_status = 1.5
                    ELSE 
                        LocalVar%GenTq = ( ( LocalVar%VS_LastGenTrq_init_ramp - &
                        ( LocalVar%GenTq_ramp ) ) / CntrPar%dt_rise * &
                        ( (LocalVar%n_DT * LocalVar%DT) - LocalVar%T_end_IS_Tar ) + &
                        (LocalVar%GenTq_ramp))
                ...
                ELSEIF ((LocalVar%IS_status == 1.5 .OR. LocalVar%IS_status == 2.5) .AND. &
                        (((LocalVar%n_DT * LocalVar%DT) - LocalVar%T_end_IS_Tar) <= CntrPar%dt_rise))THEN
                    
                    IF (LocalVar%VS_LastGenTrq_init_ramp <= LocalVar%P_mec_init) THEN
                        LocalVar%GenTq = ( ( ((LocalVar%Pmec_est - (CntrPar%VS_RtPwr * CntrPar%Frecovery)) / LocalVar%GenSpeedF) -  LocalVar%GenTq_ramp ) / CntrPar%dt_rise * &
                            ( (LocalVar%n_DT * LocalVar%DT) - LocalVar%T_end_IS_Tar ) + &
                            (LocalVar%GenTq_ramp) ) 
                    ...
                    ELSE
                        ...
                        LocalVar%IS_status = 2.5
                    END IF
                    
                    

                ELSEIF ( (LocalVar%IS_LastGenTrq > LocalVar%VS_LastGenTrq) .AND. (LocalVar%IS_status /= 3) .AND. (LocalVar%IS_status /= 2.5) .AND. (LocalVar%IS_status /= 0)) THEN
                ...
                !ELSEIF ( (LocalVar%IS_LastGenTrq > LocalVar%VS_LastGenTrq) .AND. (LocalVar%IS_status /= 3) .AND. (LocalVar%IS_status /= 0)) THEN
        ...
        IF ((CntrPar%IS_ControlMode == 5) .AND. (LocalVar%n_DT >= CntrPar%Itim_SI) .AND. (LocalVar%IS_ControlMode2 /= 1)) THEN
            ...
            IF (LocalVar%n_DT <= CntrPar%Ftim_SI) THEN
                ...
                IF ((LocalVar%GenSpeedF > CntrPar%VS_RefSpd * CntrPar%F_omega_min) .AND. (LocalVar%IS_status == 0)) THEN
                    ...
                    LocalVar%GenTq = LocalVar%GenTq_end
