...
TYPE, PUBLIC :: ControlParameters
    ...
    INTEGER(IntKi)                :: IS_ControlMode              ! FG: Synthetic inertia strategy { 0: synthetic inertia off, 1: KOmega2 mode, 2: Tarnowski mode, 3: Ontario, 4: Step wise Wang mode}
    REAL(DbKi)                    :: Ftim_SI                     ! FG: Time step where the synthetic inertia ends
    REAL(DbKi)                    :: Itim_SI                     ! FG: Time step where the synthetic inertia begins
    REAL(DbKi)                    :: F_SI                        ! FG: Overpower factor to apply --> Tarnowski mod & Ontario & KOmega2 mode
    REAL(DbKi)                    :: F_omega_min                 ! FG: Minimum rotor speed factor --> Tarnowski mod
    REAL(DbKi)                    :: Frecovery                   ! FG: Factor that affects the power of the generator in recovery --> Tarnowski mod
    REAL(DbKi)                    :: TG                          ! FG: TG: Tarnowski-Giusto
    REAL(DbKi)                    :: dt_rise                     ! Tiempo de subida y bajada de la rampa de Ontario (y ahora Tarnowski bajada)
    REAL(DbKi)                   :: end_Ontario_step            ! FG: end Ontario Step (including ramp)
    REAL(DbKi)                   :: Delta_T_over                 ! FG: Time duration of the overproduction period in GMFC strategy
    ...
    REAL(Dbki)                   :: SD_Time                     ! Shutdown Time: AGREGO FG   
...
TYPE, PUBLIC :: LocalVariables
    ...
    REAL(DbKi)                    :: GenSpeedF_dot               ! Derivative of Filtered HSS (generator) speed [rad/s^2].
    REAL(DbKi)                    :: GenSpeedF_dot_filt          !Agrega FG 11/07/25: filtro a la derivada
    ...
    REAL(DbKi)                    :: IS_status                   ! FG: Synthetic inertia status    
    REAL(DbKi)                    :: Pgen_ref                    ! FG: Generator power step (over the rated power)   
    REAL(DbKi)                    :: OmegaTi                     ! FG: Generator Speed when the SI step is applied
    REAL(DbKi)                    :: IS_ControlMode2             ! FG: Flag to determine wheather or not to enter the SI code. 
    REAL(DbKi)                    :: Pmec_est                    ! FG: Estimated mechanical power 30/06/25
    REAL(DbKi)                    :: GenSpeedF_Last              ! FG: Generator speed from previous step 30/06/25
    REAL(DbKi)                    :: IS_LastGenTrq               ! FG: Last Generator torque from IS controller
    REAL(DbKi)                    :: Last_omega                  ! FG: Omega from previous step --- WANG    
    REAL(DbKi)                    :: GenTq_Ti              ! Gent Tq at ITimSi
    REAL(DbKi)                    :: GenTq_end                   ! For GMFC strat. 
    REAL(DbKi)                    :: T_end_IS_Tar                 !FG: Time at which the SI action ends (Tarnowski)
    REAL(DbKi)                    :: T_end_rec_Tar                 !FG: Time at which the SI recovery ends (Tarnowski)
    REAL(DbKi)                    :: GenSpeedF_dot_gmfc_strategy   !FG: Derivative of filtered generator speed for the gmfc strategy.
    REAL(DbKi)                    :: GenTq_ramp                    !FG: Ramped generator torque for smooth transitions.
    REAL(DbKi)                    :: P_mec_init                    !FG: Initial mechanical power
    REAL(DbKi)                    :: VS_LastGenTrq_init_ramp       !FG: Initial last generator torque for ramping (IS status 1.5)
    !FG: 03/07/25:
    REAL(DbKi)                    :: GenSpeedF_History(3)  ! Últimos 3 valores
...
TYPE, PUBLIC :: DebugVariables
    ...
    REAL(DbKi)                    :: VS_RefSpd                   ! Torque control generator speed set point [rad/s]
    REAL(DbKi)                    :: PC_RefSpd                   ! Generator speed set point of pitch controller [rad/s]
    REAL(DbKi)                    :: IS_status                   ! IS status
    REAL(DbKi)                    :: Pmec_est                    ! Estimated mechanical power [W]
    REAL(DbKi)                    :: GenSpeedF_dot               ! Derivative of Filtered HSS (generator) speed [rad/s^2].
    REAL(DbKi)                    :: Tau_r
    REAL(DbKi)                    :: VS_GenTq
    REAL(DbKi)                    :: VS_SpdErr
