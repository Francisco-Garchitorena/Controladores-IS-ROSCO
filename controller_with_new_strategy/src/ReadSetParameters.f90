...
! -----------------------------------------------------------------------------------
    ...
    SUBROUTINE ReadControlParameterFileSub(CntrPar, LocalVar, accINFILE, accINFILE_size, RootName, ErrVar)!, accINFILE_size)
        ...
        CALL ParseInput(FileLines,'IS_ControlMode',        CntrPar%IS_ControlMode,          accINFILE(1), ErrVar, UnEc=UnEc)
        IF (ErrVar%aviFAIL < 0) RETURN
        ...
        !------------ SYNTHETHIC INERTIA ------------
        CALL ParseInput(FileLines,  'F_SI',        CntrPar%F_SI,      accINFILE(1),   ErrVar, CntrPar%IS_ControlMode == 0, UnEc)
        CALL ParseInput(FileLines,  'Ftim_SI',    CntrPar%Ftim_SI,  accINFILE(1),   ErrVar, CntrPar%IS_ControlMode == 0, UnEc)
        CALL ParseInput(FileLines,  'Itim_SI',          CntrPar%Itim_SI,        accINFILE(1),   ErrVar, CntrPar%IS_ControlMode == 0, UnEc)
        CALL ParseInput(FileLines,  'F_omega_min',          CntrPar%F_omega_min,        accINFILE(1),   ErrVar, CntrPar%IS_ControlMode == 0, UnEc)
        CALL ParseInput(FileLines,  'Frecovery',          CntrPar%Frecovery,        accINFILE(1),   ErrVar, CntrPar%IS_ControlMode == 0, UnEc)
        CALL ParseInput(FileLines,  'TG',          CntrPar%TG,        accINFILE(1),   ErrVar, CntrPar%IS_ControlMode == 0, UnEc)
        CALL ParseInput(FileLines,  'end_Ontario_step',          CntrPar%end_Ontario_step,        accINFILE(1),   ErrVar, CntrPar%IS_ControlMode == 0, UnEc)
        CALL ParseInput(FileLines,  'dt_rise',          CntrPar%dt_rise,        accINFILE(1),   ErrVar, CntrPar%IS_ControlMode == 0, UnEc)
        CALL ParseInput(FileLines,  'Delta_T_over',          CntrPar%Delta_T_over,        accINFILE(1),   ErrVar, CntrPar%Delta_T_over == 0, UnEc)
