...
if First_time
  ...
  %  save(sprintf('Simulations_data_ALL_%s.mat',Turbine));
else
    load(sprintf('Simulations_data_ALL_%s.mat',Turbine))
    Pnom =3370;
    for v = 1:length(velocidades)
...
for im = 1:nMetrics
   ...
   % ylabel(metrics_labels{im},'Interpreter','latex');
    ...
    if im ==3
        legend('Stepwise','Torque-limit','Location','northwest','FontSize',fs-2,'Interpreter','latex')
    end
    for ic = 1:numel(codes)
        G = GridCodes.(codes{ic});
