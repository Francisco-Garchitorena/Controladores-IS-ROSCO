%% Script para graficar scatter de Max o DEL vs velocidad y agrega linea punteada de valores maximos de ATLAS
%clear; clc;
clear; clc; close all;
...
%%
%% === Cargar máximos globales (de script previo) ===
maxGlobalTable = readtable('ATLAS_IEA3p4MW/MaxIndividual_Global_por_Variable.csv');
...
for vvar = 1:nVar
    ...
    % === Agregar línea horizontal con el máximo global entre velocidades ===
    % Buscar coincidencia parcial (por ejemplo "RootMxb1" dentro de "RootMxb1_[kN-m]")
    idxMatch = find(contains(maxGlobalTable.Variable, var, 'IgnoreCase', true));
    
    if ~isempty(idxMatch)
        y_max_global = maxGlobalTable.MaxIndividual_Global(idxMatch(1)); % por si hay más de una
        yline(y_max_global, '--k', 'LineWidth', 1.5, 'DisplayName','Max global');
    else
        % Aviso opcional si no encuentra coincidencia
        fprintf('⚠ No se encontró coincidencia para variable "%s" en la tabla de máximos.\n', var);
    end
    
    %Ajusto limites
    ylim_max = max([ylim_max, y_max_global + 100]);
           
    % === Ajustes de ejes ===
    set(gca,'XTick',1:length(velocidades),'XTickLabel',velnums_tmp,'FontSize',fs,'TickLabelInterpreter','latex');
    ylim([ylim_min, ylim_max]);

    % === Leyenda sólo en un subplot (opcional) ===
    if vvar == 3
        lgd_handles = gobjects(length(estrategias)+1,1);
        for e = 1:length(estrategias)
            lgd_handles(e) = plot(NaN, NaN, 's', 'MarkerFaceColor', colors(e,:), 'MarkerEdgeColor', colors(e,:));
        end
        lgd_handles(end) = plot(NaN, NaN, '--k', 'LineWidth', 1.5); % línea punteada global
        legend(lgd_handles, [estrategia_names, "ATLAS 24 seeds DC max"], 'Location','best','Interpreter','latex');
    end
end
...
resultados_csv = readtable('ATLAS_IEA3p4MW/ATLAS_IEA3p4MW_resultados_para_torque26.csv');

% Limpieza de nombres de columnas si es necesario
resultados_csv.Properties.VariableNames = strrep(resultados_csv.Properties.VariableNames, ' ', '_');
resultados_csv.Properties.VariableNames = strrep(resultados_csv.Properties.VariableNames, '.', '_');
resultados_csv.Properties.VariableNames = strrep(resultados_csv.Properties.VariableNames, '-', '_');
...
for vvar = 1:length(variables)
    ...
    % --- Agregar línea punteada horizontal para SeedSpeedWeightedDEL ---
    % Filtrar la fila correspondiente a la variable actual
    idx_var =  find(contains(resultados_csv.Variable, var, 'IgnoreCase', true));
    
    if any(idx_var)
        seedSpeedVal = mean(resultados_csv.SeedSpeedWeightedDEL(idx_var),'omitnan');
        yline(seedSpeedVal/ref8,'--r','LineWidth',1.5,'DisplayName','SeedSpeedWeightedDEL');
    end
