...
def run_turbsim_cases(root_folder, DLC_folder_name, Wind_file_root_name, velocidades, DLCs, seeds, TI_SSW, sel_nucleos, turb_sel, TurbSim_DLCs, variation_name, openfast_exe, TI):
    """
    ...
    print(velocidades)
    args_list = [
        (f'{base_dlc_path}/Wind/SPW/v{vel}/TI{TI}.0/sd{seeds.index(sd)}/{Wind_file_root_name}_{vel}_sh{sh}_TI{TI}.0_sd{seeds.index(sd)}.inp')
        for vel in velocidades for sd in seeds]
    print(args_list)
